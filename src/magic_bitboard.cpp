 ...
 */
int MagicBitboard::bitScanForward(
 ...
 */
int MagicBitboard::bitScanReverse(
  std::uint64_t bb) const
{
  if (__cplusplus >= 202002L)
  {
...
void MagicBitboard::generateMoves(
...
BoardState MagicBitboard::getBoardState() const
{
  return mBoardState;
}
...
bool MagicBitboard::isCellAttacked(
  std::int8_t row,
  std::int8_t col,
...
bool MagicBitboard::isKingInCheck(
  Color color) const
{
...
void MagicBitboard::setPosition(
...
void MagicBitboard::updateAggregateBitboards()
{
  mWhitePieces = 0;
  mBlackPieces = 0;
  for (std::int32_t i = PAWN_INDEX; i <= KING_INDEX; i++)
...
void MagicBitboard::writeBitBoard(
  std::uint64_t bb,
